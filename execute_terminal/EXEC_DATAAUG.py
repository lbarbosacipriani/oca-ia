...
path_out = 'saida_4000-augumented/saidas/'
...
for i in tqdm(range(len(img_dataset)), desc='Applying augmentation'):
    ...
    _elem_aux = pd.DataFrame({'image': [img_dataset_aug[i]], 'label': int(_label[i])})
    df_aug = pd.concat([df_aug, _elem_aux], ignore_index=True)
    _elem_aux_orig = pd.DataFrame({'image': [img_dataset[i]], 'label': int(_label[i])})
...
for i, (train_index, test_index) in enumerate(kf.split(train_dataset)):
    ...
    if (flg_salvar_modelos):
        salvar_model(model, path='4000-files-augument/output/', name_file=f'model_fold_{i}.pth')

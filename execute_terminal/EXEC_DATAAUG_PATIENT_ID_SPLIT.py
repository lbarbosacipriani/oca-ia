## execucao dia 14 de maio de 2026 - data augumentation# 🔧 DIAGNÓSTICO DE ERRO CUDA - Execute antes de treinar
import torch
...
FILE_PATH = '/home/leo/Documents/dev/oca-ia/data_balanced_AFIB_OTHERS_Patient_id.csv'
...
path_out = 'saida_4000-augumented-2/saidas/'
path_models = 'saida_4000-augumented-2/modelos/'
...
df_aug = pd.DataFrame(columns=['image','label'])
df_orig = pd.DataFrame(columns=['image','label'])
_label = data['label']
for i in tqdm(range(len(img_dataset)), desc='Applying augmentation'):
    ...
    #print(img_dataset_aug[i])
    #print(tensor_label[i])    
    _elem_aux = pd.DataFrame({'image': [img_dataset_aug[i]], 'label': int(_label[i]), 'patient_id': data['patient_id'][i]})
    df_aug = pd.concat([df_aug, _elem_aux], ignore_index=True)
    _elem_aux_orig = pd.DataFrame({'image': [img_dataset[i]], 'label': int(_label[i]), 'patient_id': data['patient_id'][i]})
    
    df_orig = pd.concat([df_orig, _elem_aux_orig], ignore_index=True)
df_full = pd.concat([df_orig, df_aug], ignore_index=True)

## Rewrite images and tensor with augumented values.
img_dataset_out = np.concatenate((img_dataset, img_dataset_aug), axis=0)
tensor_imagem = torch.from_numpy(img_dataset_out)
tensor_label = df_full['label'].values
data = df_full
...
print("Distribuicao das amostras")
...
for i, (train_index, test_index) in enumerate(kf.split(patient_id_list)):
    ...
    train_dataset_part = Subset(train_dataset, dados_train.index)
    val_dataset_part = Subset(train_dataset, dados_val.index)
